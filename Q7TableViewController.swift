...
class Q7TableViewController: QuestionTableViewController, CellProtocol {
    ...
    func switchButtonTapped(WithStatus status: Bool, ForCell myCell: QuestionsTableViewCell) {
        ...
        let ch7SwitchSelected = myCell.questionLabel.text!
        print("Question added/removed was \(String(describing: ch7SwitchSelected))")
    ...
    func checkForSavedSelectedQuestions() {
        let savedQuestions = QuestionStorage.shared.getSelectedQuestions(by: .ch7)
...
//        print(UserDefaults.standard.dictionaryRepresentation())
        ...
        guard let url = Bundle.main.url(forResource: "ch7Questions", withExtension: "json"),
            let ch7JSONData = try? Data(contentsOf: url),
            let ch7questions = try? JSONDecoder().decode(Array<Question>.self, from: ch7JSONData) else {
                print("Error loading JSON")
                fatalError()
        }

        self.questions = ch7questions
