...
class Q8TableViewController: QuestionTableViewController, CellProtocol {
    ...
    func switchButtonTapped(WithStatus status: Bool, ForCell myCell: QuestionsTableViewCell) {
        ...
        let ch8SwitchSelected = myCell.questionLabel.text!
        print("Question added/removed was \(String(describing: ch8SwitchSelected))")
    ...
    func checkForSavedSelectedQuestions() {
        let savedQuestions = QuestionStorage.shared.getSelectedQuestions(by: .ch8)
...
//        print(UserDefaults.standard.dictionaryRepresentation())
        ...
        guard let url = Bundle.main.url(forResource: "ch8Questions", withExtension: "json"),
            let ch8JSONData = try? Data(contentsOf: url),
            let ch8questions = try? JSONDecoder().decode(Array<Question>.self, from: ch8JSONData) else {
                print("Error loading JSON")
                fatalError()
        }

        self.questions = ch8questions
