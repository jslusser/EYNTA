...
class Q13TableViewController: QuestionTableViewController, CellProtocol {
    ...
    func switchButtonTapped(WithStatus status: Bool, ForCell myCell: QuestionsTableViewCell) {
        ...
        let ch13SwitchSelected = myCell.questionLabel.text!
        print("Question added/removed was \(String(describing: ch13SwitchSelected))")
    ...
    func checkForSavedSelectedQuestions() {
        let savedQuestions = QuestionStorage.shared.getSelectedQuestions(by: .ch13)
...
//        print(UserDefaults.standard.dictionaryRepresentation())
        ...
        guard let url = Bundle.main.url(forResource: "ch13Questions", withExtension: "json"),
            let ch13JSONData = try? Data(contentsOf: url),
            let ch13questions = try? JSONDecoder().decode(Array<Question>.self, from: ch13JSONData) else {
                print("Error loading JSON")
                fatalError()
        }

        self.questions = ch13questions
