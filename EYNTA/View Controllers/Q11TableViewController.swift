...
class Q11TableViewController: QuestionTableViewController, CellProtocol {
    ...
    func switchButtonTapped(WithStatus status: Bool, ForCell myCell: QuestionsTableViewCell) {
        ...
        let ch11SwitchSelected = myCell.questionLabel.text!
        print("Question added/removed was \(String(describing: ch11SwitchSelected))")
    ...
    func checkForSavedSelectedQuestions() {
        let savedQuestions = QuestionStorage.shared.getSelectedQuestions(by: .ch11)
...
//        print(UserDefaults.standard.dictionaryRepresentation())
        ...
        guard let url = Bundle.main.url(forResource: "ch11Questions", withExtension: "json"),
            let ch11JSONData = try? Data(contentsOf: url),
            let ch11questions = try? JSONDecoder().decode(Array<Question>.self, from: ch11JSONData) else {
                print("Error loading JSON")
                fatalError()
        }

        self.questions = ch11questions
