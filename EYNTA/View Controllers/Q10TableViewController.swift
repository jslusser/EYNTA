...
class Q10TableViewController: QuestionTableViewController, CellProtocol {
    ...
    func switchButtonTapped(WithStatus status: Bool, ForCell myCell: QuestionsTableViewCell) {
        ...
        let ch10SwitchSelected = myCell.questionLabel.text!
        print("Question added/removed was \(String(describing: ch10SwitchSelected))")
    ...
    func checkForSavedSelectedQuestions() {
        let savedQuestions = QuestionStorage.shared.getSelectedQuestions(by: .ch10)
...
//        print(UserDefaults.standard.dictionaryRepresentation())
        ...
        guard let url = Bundle.main.url(forResource: "ch10Questions", withExtension: "json"),
            let ch10JSONData = try? Data(contentsOf: url),
            let ch10questions = try? JSONDecoder().decode(Array<Question>.self, from: ch10JSONData) else {
                print("Error loading JSON")
                fatalError()
        }

        self.questions = ch10questions
